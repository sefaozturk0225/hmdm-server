...
public class QRCodeResource {
    ...
    // ── Device Owner provisioning QR for com.tahirova.launcher ─────────────
    @ApiOperation(
            value = "DO provisioning QR",
            notes = "Generates a Device Owner provisioning QR code for Tahirova Launcher.",
            responseHeaders = {@ResponseHeader(name = "Content-Type")}
    )
    @GET
    @Path("/do-provisioning")
    @Produces(MediaType.APPLICATION_OCTET_STREAM)
    public javax.ws.rs.core.Response generateDoProvisioningQR(
            @QueryParam("apkUrl") @ApiParam("APK download URL") String apkUrl,
            @QueryParam("checksum") @ApiParam("Pre-calculated SHA-256 base64 checksum") String checksum,
            @QueryParam("size") @ApiParam("QR image size") Integer size) {

        if (apkUrl == null || apkUrl.trim().isEmpty()) {
            return javax.ws.rs.core.Response.status(400).entity("apkUrl is required").build();
        }
        ...
        try {
            ...
            if (checksum != null && !checksum.trim().isEmpty()) {
                ...
                sha256 = calculateApkHash(apkUrl.trim());
            }

            String json = "{\n" +
                    "\"android.app.extra.PROVISIONING_DEVICE_ADMIN_COMPONENT_NAME\":" +
                    "\"com.tahirova.launcher/.OdakDeviceAdmin\",\n" +
                    "\"android.app.extra.PROVISIONING_DEVICE_ADMIN_PACKAGE_DOWNLOAD_LOCATION\":" +
                    JSONObject.quote(apkUrl.trim()) + ",\n" +
                    "\"android.app.extra.PROVISIONING_DEVICE_ADMIN_PACKAGE_CHECKSUM\":\"" + sha256 + "\",\n" +
                    "\"android.app.extra.PROVISIONING_ORGANIZATION_NAME\":\"Detoks Telefonu\",\n" +
                    "\"android.app.extra.PROVISIONING_LEAVE_ALL_SYSTEM_APPS_ENABLED\":true,\n" +
            ...
            logger.info("DO provisioning QR generated for APK: {}", apkUrl);

            final int imageSize = size != null ? size : 400;
            return javax.ws.rs.core.Response.ok((StreamingOutput) output -> {
                try {
                    QRCode.from(json).withCharset("UTF-8")
                            .to(ImageType.PNG)
            ...
            logger.error("Error generating DO provisioning QR: {}", e.getMessage());
            return javax.ws.rs.core.Response.serverError()
                    .entity("Checksum hesaplanamadı: " + e.getMessage()).build();
        }
    }

    // JSON versiyonu (textarea'da göstermek için)
    @GET
    @Path("/do-provisioning/json")
    @Produces(MediaType.APPLICATION_JSON)
    public javax.ws.rs.core.Response generateDoProvisioningJSON(
            @QueryParam("apkUrl") @ApiParam("APK download URL") String apkUrl,
            @QueryParam("checksum") @ApiParam("Pre-calculated SHA-256 base64 checksum") String checksum) {
        ...
        try {
            ...
            return javax.ws.rs.core.Response.serverError()
                    .entity("Hata: " + e.getMessage()).build();
        }
    }
